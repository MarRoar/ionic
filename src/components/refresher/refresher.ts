 ...
 * <ion-content>
 *
 *   <ion-refresher (refresherRefresh)="doRefresh($event)">
...
export class Refresher {
   ...
   */
  @Output() refresherRefresh: EventEmitter<Refresher> = new EventEmitter();

  /**
   * @output {event} While the user is pulling down the content and exposing the refresher.
   */
  @Output() refresherPulling: EventEmitter<Refresher> = new EventEmitter();

  /**
   * @output {event} When the user begins to start pulling down.
   */
  @Output() refresherStart: EventEmitter<Refresher> = new EventEmitter();
  ...
  private _onMoveInZone() {
    ...
    if (!this._didStart) {
      this._didStart = true;
      this.refresherStart.emit(this);
    }

    // emit "pulling" on every move
    this.refresherPulling.emit(this);
  ...
  private _beginRefresh() {
    ...
    this.refresherRefresh.emit(this);
  }
