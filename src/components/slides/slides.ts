 ...
 * Let's add the `slideDidChange` event and call a method when the slide changes:
 *
 * ```html
 * <ion-slides #mySlider (slideDidChange)="onSlideChanged()" [options]="mySlideOptions">
 * ```
 *
...
export class Slides extends Ion {
   ...
   */
  @Output() slideWillChange: EventEmitter<any> = new EventEmitter();

  /**
   * @output {any} Expression to evaluate when a slide change ends.
   */
  @Output() slideDidChange: EventEmitter<any> = new EventEmitter();

  /**
   * @output {any} Expression to evaluate when a slide moves.
   */
  @Output() slideMove: EventEmitter<any> = new EventEmitter();
  ...
  */
   ...
   * @param {boolean} runCallbacks  Whether or not to emit the `slideWillChange`/`slideDidChange` events. Optional. Default true.
   */
